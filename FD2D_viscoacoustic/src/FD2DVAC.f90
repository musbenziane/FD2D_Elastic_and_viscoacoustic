program FD2D_VISCOACOUSTIC
    ...
    if (is_PML==1 .and. is_FS==1) then
    ...
    else if (is_PML .eq. 1 .and. is_FS .eq. 0) then 
        allocate(c2D(nz+2*npml,nx+2*npml))
        allocate(qf2D(nz+2*npml,nx+2*npml))
        allocate(rho2D(nz+2*npml,nx+2*npml))

        c2D(npml+1:nz+npml,npml+1:nx+npml)   = ctmp
        qf2D(npml+1:nz+npml,npml+1:nx+npml)  = qftmp
        rho2D(npml+1:nz+npml,npml+1:nx+npml) = rhotmp
        ...
        do i=1,npml
            c2D(nz+npml+i,:)      = c2D(nz,:)
            qf2D(nz+npml+i,:)     = qf2D(nz,:)
            rho2D(nz+npml+i,:)    = rho2D(nz,:)
        end do
    ...
    wPML      = dz * (npml-1)
    sigma_m   =-3. * maxval(c2D) /2. / wPML * log(1e-5);
    ...
    do is=1,nshot
        ...
        do it=1,nt
            !$OMP PARALLEL DO PRIVATE(iz,ix) SCHEDULE(static)
            do ix=5,nx-4
                do iz=5,nz-4
                    temp      = K(iz,ix)*dt*(tau_epsi(iz,ix)/tau_sigma(iz,ix)-1.0)/dz/tau_sigma(iz,ix)
                    r(iz,ix)  = (1.0-dt/tau_sigma(iz,ix))*r(iz,ix)-temp                 * &
                                (c1*(u(iz,ix)-u(iz,ix-1)+w(iz,ix)-w(iz-1,ix))           + &
                                 c2*(u(iz,ix+1)-u(iz,ix-2)+w(iz+1,ix)-w(iz-2,ix))       + &
                                 c3*(u(iz,ix+2)-u(iz,ix-3)+w(iz+2,ix)-w(iz-3,ix))       + &
                                 c4*(u(iz,ix+3)-u(iz,ix-4)+w(iz+3,ix)-w(iz-4,ix)))
                end do
            end do
            !$OMP END PARALLEL DO
            ...
            do ix=5,nx-4
                do iz=5,nz-4
                    ...
                    px(iz,ix) = (1-dt*sigmax(ix)) * px(iz,ix)-r(iz,ix)*dt-temp          * &    
                                (c1*(u(iz,ix)-u(iz,ix-1))                               + &
                                 c2*(u(iz,ix+1)-u(iz,ix-2))                             + &
                                 c3*(u(iz,ix+2)-u(iz,ix-3))                             + &
                                 c4*(u(iz,ix+3)-u(iz,ix-4)))
                end do
            end do
            !$OMP END PARALLEL DO
            ...
            p(:,:)  = pz(:,:) + px(:,:)     
            pz(isrc(is),jsrc(is))  = pz(isrc(is),jsrc(is)) + dt * src(it) 
            ...
            do ix=5,nx-4
                do iz=5,nz-4
                    ...
                    u(iz,ix) = (1-dt*sigmaz(ix)) * u(iz,ix) - temp                      * &
                                    (c1*(p(iz,ix+1)-p(iz,ix))                           + &
                                     c2*(p(iz,ix+2)-p(iz,ix-1))                         + &  
                                     c3*(p(iz,ix+3)-p(iz,ix-2))                         + &
                                     c4*(p(iz,ix+4)-p(iz,ix-3)))

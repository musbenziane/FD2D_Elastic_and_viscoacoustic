program FD2D_VISCOACOUSTIC
    !####################################################################################################################
    ! FD2D: Finite difference solver for the visco(acoustic) wave equations in cartesian coordinates with flat free surface.
    ! NOTE: This program has not been validated.
    ! 
    ! Language: Fortran 90, with parralel impelementation using OpenMP API
    ! 
    ! Author: Mus Benziane
    ! Source: Virieux 1986 add ***
    ...
    do is=1,nshot
        ...
        do it=1,nt
            ...
            if (srctype==1) then    ! Explosive source
                sigma(isrc(is),jsrc(is)) = sigma(isrc(is),jsrc(is)) + .5*dt * src(it)
                tau(isrc(is),jsrc(is))   = tau(isrc(is),jsrc(is))   + .5*dt * src(it)
